...
class AnomalyDetectionConsumer:
    def __init__(self, topic, bootstrap_servers, db_uri, db_name, collection_name, output_file):
        ...
        self.transactions = []
        self.window_size = 50
        self.low_value_threshold = 1.0  # Threshold for low value anomaly detection
    ...
    def update_plot(self):
        ...
        # Highlight anomalies
        anomaly_indices = [idx - 1 for idx in self.anomalies if idx - 1 < len(self.amounts)]
        anomaly_values = [self.amounts[i] for i in anomaly_indices]
        self.ax.scatter(anomaly_indices, anomaly_values, color='red', label='Anomalies')
        self.ax.legend()
        plt.draw()
        plt.pause(0.01)
    ...
    def detect_anomalies(self, new_transaction, idx):
        ...
        if len(self.transactions) >= 30:
            ...
            if (new_transaction['amount'] > upper_bound) or (new_transaction['amount'] < lower_bound) or (new_transaction['amount'] < self.low_value_threshold):
                anomaly_msg = f"Anomaly detected: Transaction ID {new_transaction['transaction_id']} with amount {new_transaction['amount']}"
                print(anomaly_msg)
